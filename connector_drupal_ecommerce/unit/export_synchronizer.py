...
"""
Exporters for Drupal heavy based on Magento.
In addition to its export job, an exporter has to:
* check in Drupal if the record has been updated more recently than the
  last sync date and if yes, delay an import
* call the ``bind`` method of the binder to update the last sync date
"""


class DrupalBaseExporter(ExportSynchronizer):
    """ Base exporter for Drupal """
    ...
    def __init__(self, environment):
        ...
        """
        super(DrupalBaseExporter, self).__init__(environment)
    ...
    def _delay_import(self):
        ...
        import_record.delay(
            ...
            self.drupal_id, force=True
        )

    def _should_import(self):
        """ Before the export, compare the update date
        in Drupal and the last sync date in OpenERP,
        if the former is more recent, schedule an import
        to not miss changes done in Drupal.
        """
    ...
    def run(self, binding_id, *args, **kwargs):
        ...
        self.drupal_id = self.binder.to_backend(self.binding_id)
        try:
        ...
        self.binder.bind(self.drupal_id, self.binding_id)
...
class DrupalExporter(DrupalBaseExporter):
    """ A common flow for the exports to Drupal """
    ...
    def __init__(self, environment):
        ...
        :type environment: :py:class:`connector.connector.Environment`
        """
        super(DrupalExporter, self).__init__(environment)
    ...
    def _after_export(self):
        """ Can do several actions after exporting a record on drupal """
        pass
    ...
    def _update(self, data):
        ...
        self.backend_adapter.write(self.drupal_id, data)
    ...
    def _run(self, fields=None):
        ...
        if self.drupal_id:
            ...
            if not record:
                return _('Nothing to export.')
            self.drupal_id = self._create(record)
        return _('Record exported with ID %s on Drupal.') % self.drupal_id


@job
@related_action(action=unwrap_binding)
def export_record(session, model_name, binding_id, fields=None):
    """ Export a record to Drupal """
    record = session.browse(model_name, binding_id)
    env = get_environment(session, model_name, record.backend_id.id)
    exporter = env.get_connector_unit(DrupalExporter)
